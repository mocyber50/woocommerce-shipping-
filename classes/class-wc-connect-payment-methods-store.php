...
class WC_Connect_Payment_Methods_Store {
	...
	/**
	 * Get the first payment method ID from the available payment methods.
	 *
	 * @return int The first payment method ID, or 0 if none available.
	 */
	public function get_first_payment_method_id() {
	...
	/**
	 * Get payment method ID for label purchase, with optional autoselect.
	 * If autoselect is enabled and no payment method is selected, returns the first available one.
	 *
	 * @param bool $autoselect Whether to automatically select the first payment method if none selected.
	 * @return int The payment method ID to use.
	 */
	public function get_payment_method_id_for_purchase( $autoselect = false ) {
		$payment_method_id = $this->service_settings_store->get_selected_payment_method_id();

		// If autoselect is enabled and no payment method is selected, use the first available one
		if ( $autoselect && 0 === $payment_method_id ) {
			$payment_method_id = $this->get_first_payment_method_id();
		}
	 ...
	 * @return bool|WP_Error Whether the response body is valid.
	 */
	protected function validate_payment_methods_response( $response_body ) {
		...
		if ( ! property_exists( $response_body, 'add_payment_method_url' ) ) {
			return new WP_Error( 'payment_method_response_body_missing_add_payment_method_url', 'Expected but did not receive add_payment_method_url in response body.' );
		}
	 ...
	 *
	 * @param object $response_body The response body object.
	 * @return string|WP_Error The URL to add a payment method.
	 */
